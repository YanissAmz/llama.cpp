...
void llama_model_bailingmoe3::load_arch_hparams(llama_model_loader & ml) {
    ...
    if (hparams.n_ff_shexp == 0) {
        hparams.n_ff_shexp = hparams.n_ff_exp_impl * std::max(1u, hparams.n_expert_shared);
    }
...
void llama_model_bailingmoe3::load_arch_tensors(llama_model_loader & ml) {
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if ((uint32_t) il < hparams.n_layer_dense_lead) {
            ...
            layer.ffn_gate_exps = create_tensor(tn(LLM_TENSOR_FFN_GATE_EXPS, "weight", il), { n_embd, hparams.n_ff_exp_impl, n_expert }, trunk_flags);
            layer.ffn_up_exps = create_tensor(tn(LLM_TENSOR_FFN_UP_EXPS, "weight", il), { n_embd, hparams.n_ff_exp_impl, n_expert }, trunk_flags);
            layer.ffn_down_exps = create_tensor(tn(LLM_TENSOR_FFN_DOWN_EXPS, "weight", il), { hparams.n_ff_exp_impl, n_embd, n_expert }, trunk_flags);
    ...
    for (int il = n_layer; il < n_layer_all; ++il) {
        ...
        layer.ffn_gate_exps = create_tensor(tn(LLM_TENSOR_FFN_GATE_EXPS, "weight", il), { n_embd, hparams.n_ff_exp_impl, n_expert }, flags);
        layer.ffn_up_exps = create_tensor(tn(LLM_TENSOR_FFN_UP_EXPS, "weight", il), { n_embd, hparams.n_ff_exp_impl, n_expert }, flags);
        layer.ffn_down_exps = create_tensor(tn(LLM_TENSOR_FFN_DOWN_EXPS, "weight", il), { hparams.n_ff_exp_impl, n_embd, n_expert }, flags);
